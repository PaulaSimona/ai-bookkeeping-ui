    ...
    <PageShell>
      ...
      <div className="mt-6">
        <Card>
          ...
          {/* Persistent archived-tab explainer (D-14C2-9): the SAME copy as the
              empty state, read from the one config value so the two surfaces can
              never drift. Shows whenever the Archived tab is active — with rows
              or without. */}
          {archivedTab && (
            <div className="flex items-start gap-2 border-b border-gray-100 bg-gray-50 px-4 py-2.5 text-[12.5px] text-gray-600">
              ...
              <span>{config.emptyArchived}</span>
            </div>
          )}
